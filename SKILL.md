...
1. **宏观天气预报与历史档案检索 (Macro & Delta)**：**强制读取** `advanced_context.json` 里的宏观指标。如果宏观天气为“暴风雨”（如 PMI < 50 且 M1增速 < M2），必须在报告开头亮起宏观红灯，并对周期股/高估值成长股实行“最高 B 级”的评级封杀。同时在当前工作区检索是否有该股的旧报告，作为本轮 Delta 对比的基础。
2. **制定研究树**：拆解研究维度。
