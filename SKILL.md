...
- **无用的垃圾必须销毁**：在调研和多 Agent 协同过程中随意产生的临时脚本（如临时 `.py`）、通信日志、中间态文本（`.txt`）、未命名的草稿等，以及 **所有调用 API 产生的临时 JSON 文件（如 quote.json, kline.json 等，唯独 `ledger.json` 除外）**，**必须在归档完成后彻底删除（清理）**。绝不允许将爬虫废料残留在业务数据区。
