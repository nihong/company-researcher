...
10. **红队攻击（目标函数硬切换）**：严格按照 `china_market/red_team_framework.md` 的七维攻击执行。**【强制人格切换】**：此时你必须切换目标函数，不再是“试图平衡多空的分析师”，而是“以推翻主报告逻辑为唯一 KPI 的风控总监”。如果你的红队攻击只是“形式主义的走过场（如：虽然有风险但我仍认为值得买）”，Firewall 将强制拦截。
