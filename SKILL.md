...
11. **投资决策矩阵与融资融券**：给出实战策略（左侧布局/右侧确认/等待观察/清仓）。**必须增加【融资融券适配】模块**：对于 S/A 级标的，评估是否适合用融资加仓及对应杠杆止损位；对于 B/C 级或空头逻辑标的，评估融券做空的可行性（可借券量/融券成本）。
...
- **融资融券适配策略 (v3.9 强制)**：明确指出标的是否适合杠杆做多（S/A级）或融券做空（B/C/D级），并提示相应杠杆爆仓止损线。
