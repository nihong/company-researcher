...
2. **严防幻觉的数据采集机制**：
   - 必须优先且**仅使用** `python scripts/fetch_market_data.py` 获取财务与量价数据（其内部已包含多源轮询）。
...
1. **调用风控子智能体**：调用 `invoke_subagent` 唤醒 `red_team_reviewer` (红队风控总监) 智能体，把您的初稿逻辑发给它进行审阅。**绝对禁止使用旧版的 `opencli deepseek ask` 命令（极易超时断联）。**
2. **红蓝双向辩论与动态评级 (Debate & Defense)**：
   - 接收到红队的【三大灵魂拷问】后，主 Agent 必须在全量上下文中寻找客观数据进行**正面防守/自证**。
   - **评级动态调整**：若能用硬数据完美击碎红队质疑，则标记为【预期差机会】，并上调投资评级；若无法用实锤数据反驳红队攻击（如无法解释股东减持、订单下滑），则必须在最终决策中**强制下调评级**（如从“买入”降至“观望”）。
